...
client.on('message', async (message) => {
  ...
  if (command === 'editors') {
    ...
    let embed = new Discord.MessageEmbed().setTitle(
      '__Commonly used videdo editing softwares__'
    );

    list.forEach((x) =>
      embed.addField(x.name, `**Link:** ${x.link}\n**Price:** ${x.price}\n\n`)
    );
  ...
  if (command === 'status') {
    ...
    if (args[0] === 'smp') {
      const lektonicdata = await fetch(
        'https://api.mcsrvstat.us/2/ytc.lektonic.com'
      ).then((response) => response.json());
      const worlddata = await fetch(
        'https://api.mcsrvstat.us/2/play.ytc.world'
      ).then((response) => response.json());
      let lektonic;
      let world;
      let lektonicmotd;
      ...
      if (!lektonicdata.online) {
        ...
        if (lektonicdata.motd.clean[0])
          lektonicmotd = lektonicdata.motd.clean[0];
        if (lektonicdata.motd.clean[1])
          lektonicmotd = lektonicmotd + `\n` + lektonicdata.motd.clean[1];
